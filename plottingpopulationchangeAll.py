...
alphas = [1.0]#1.6, 1.7, 1.8, 1.9, 2.0, 2.1, 2.2, 2.3]
#alphas = [1.2]
nums = [0, 1, 10]
for number, alpha in enumerate(alphas):
    ...
    np.save(f"/ddn/data/tffd79/cornwallGaussianPopDist/popAreas/times_Gaussian_{name_template}_{num}.npy", ns)
    #print(ns)
    ...
    cornwallMask = np.load("Cornwall_data/cornwall_county_mask_w_border_rightway.npy").astype(bool)
    #countyPopulation = np.load("Cornwall_data/cornwall_smoothed_pop_dist2.npy")
    countyPopulation = np.load("Cornwall_data/smoothed_PopDistnew10_2.npy")
    countyPopulation[~cornwallMask] = np.nan
    flatPop = np.ones((countyPopulation.shape[0], countyPopulation.shape[1]))
    flatPop[~cornwallMask] = np.nan
    flatPopTot = np.nansum(flatPop[cornwallMask])
    ...
    for i, n in enumerate(ns):
        ...
        pop1.append(np.nansum(countyPopulation[locs]))
    ...
    np.save(f"/ddn/data/tffd79/cornwallGaussianPopDist/popAreas/pop1_Gaussian_2_{name_template}_{num}.npy", pop1)
    np.save(f"/ddn/data/tffd79/cornwallGaussianPopDist/popAreas/area1_Gaussian_2_{name_template}_{num}.npy", area1)
